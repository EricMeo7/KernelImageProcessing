        		...
        		int filterWidth, int filterHeight)
{
	...
	filterImageShared<<<blocksPerGrid, threadsPerBlock, sharedMemorySize>>>(d_sourceImagePtr, d_outImagePtr,paddedWidth, paddedHeight,blockWidth, blockHeight,surroundingPixels,width, height,filterWidth, filterHeight);

	err = cudaGetLastError();
	if (err != cudaSuccess) {
		std::cerr << "CUDA error: " << cudaGetErrorString(err) << std::endl;
				std::cerr << "qui 2"<<std::endl;
		return false;
	}
