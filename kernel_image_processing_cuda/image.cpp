...
std::vector<float> Image::applyFilterCommon(const Kernel& kernel) const
{
    ...
    auto paddingDuration = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
    ...
    auto filterDuration = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
...
bool Image::imageProcessing(Image& resultingImage, const Kernel& kernel, const CudaMemType cudaType)
{
    ...
    std::cout << "Applying multithread filter to image" << " " << filterWidth << "x" << filterHeight << std::endl;
